...
| Faults the instant a drive starts | Step too big — current spikes past the 3 A protection. Command a smaller step or ramp it host-side; the accel byte's direction is undocumented, so a bigger value may make it worse. Auto-resets in ~5 s. | [why]({{< relref "faq" >}}#fault-on-start) |
