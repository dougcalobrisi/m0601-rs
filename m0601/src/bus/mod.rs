...
impl<T: Transport> Bus<T> {
    ...
    /// Defaults to `5` (see [`BusTiming`]) — but note that sweeping this byte
    /// across its whole range changes an unloaded stop by 1 RPM, so it is
    /// very likely inert. It is kept and still sent because the measurement is
    /// one motor on one firmware, not because tuning it is known to help.
    #[must_use]
